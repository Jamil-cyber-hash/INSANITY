        ...
        <?php
            ...
            while ($row = $result->fetch_assoc()) {
                $full_name = $row['deceased_first_name'] . ' ' . 
                             (!empty($row['deceased_middle_name']) ? $row['deceased_middle_name'] . ' ' : '') . 
                             $row['deceased_last_name'];

                // Ensure image path is correct
                $image_path = !empty($row['image']) ? 'uploads/' . $row['image'] : 'imaages/default.jpg';

                // Check if the file exists, otherwise use default image
                if (!file_exists($image_path)) {
                    $image_path = 'imaages/default.jpg';
                }
        ?>
            <div class="col-md-3 mb-4">
                <div class="card">
                    <img src="<?= htmlspecialchars($image_path) ?>" class="card-img-top" alt="<?= htmlspecialchars($full_name) ?>" style="height: 250px; object-fit: cover;">
                    <div class="card-body">
                        <h5 class="card-title"><?= htmlspecialchars($full_name) ?></h5>
                        <p><strong>Date:</strong> <?= htmlspecialchars($row['date_of_passing']) ?></p>
